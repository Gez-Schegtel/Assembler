...
;
; Aclaración: Es igual a la versión 7.3, pero ahora le di un formato diferente a la salida.
...
ingresar_vector:
  ...
  mov [di], cx
  add di, 2 ; Acordate que tenemos que avanzar 2 porque tenemos words (16 bits). Inc es para 8 bits.
...
recorrer_vector:
  lodsw ; Esta función toma lo que tenga el registro SI y hace lo siguiente en un solo ciclo: mov al, [si] , add si, 2
...
salto_contador:
  ...
  cmp cx, 1 ; Vemos si estamos ante el último número del vector. De ser así, saltamos la coma.
  je saltar_coma
